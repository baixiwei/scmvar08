...
<p>This experiment involved various combinatorics problems. All shared the same mathematical structure – that of a Sampling with Replacement problem. You received instruction in how to solve such problems, in the form of a series of worked examples. The worked examples either included two different types of situation – "different people choosing from a set of options" and "sequence of options selected from a fixed set" – or included only one of these two types. You were then tested on problems involving several new types of situation. We hypothesize that instruction using two different types of situation will lead to better subsequent performance than instruction using only one type.</p>\
